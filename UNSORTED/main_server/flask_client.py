...
def gather_img_objDetect():
    ...
    objDetect_stub = objDetect_pb2_grpc.objDetectorStub(
        grpc.insecure_channel("object_detection_service:" + str(CFG["OBJDETECT_PORT"]))
    )  # TODO object_detection_service
...
def gather_img_poseEst():
    ...
    poseEst_stub = poseEst_pb2_grpc.poseEstorStub(
        grpc.insecure_channel("pose_estimation_service:" + str(CFG["POSEEST_PORT"]))
    )
...
def gather_img_MOT(plot_in_rviz):
    ...
    """
    stub = pb2_grpc.GreeterStub(grpc.insecure_channel("localhost:" + str(CFG["CAMERA_PORT"])))
    MOT_stub = newMOT_pb2_grpc.MOTorStub(
        grpc.insecure_channel("mot_service:" + str(CFG["MOT_PORT"]))
    )
    id = 0
    cropped_bytes = 0
    exist_bytes = False
