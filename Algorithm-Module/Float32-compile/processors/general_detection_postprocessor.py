...
# decoded_outputs is size: batch_size x num_anchors_after_top_k x [x_min_abs, y_min_abs, x_max_abs, y_max_abs, top_class_index, top_class_score]
def post_process(
    decoded_outputs, iou_threshold, max_det_per_image, prediction_confidence_threshold, k=None,
):
    """
    Args
        decoded_outputs: the decoded box proposals in Efficientdet format. 
    ...
    decoded_outputs = from_n_class_to_top_class(decoded_outputs)

    batch_size = decoded_outputs.shape[0]
    batch_detections = []
    for i in range(batch_size):
        sample_decoded_outputs = decoded_outputs[i]

        if k:
            _, indeces = torch.topk(sample_decoded_outputs[:, 5], k, sorted=False)
            sample_decoded_outputs = sample_decoded_outputs[indeces]

        boxes, classes, scores = sample_decoded_outputs[:, :4], sample_decoded_outputs[:, 4], sample_decoded_outputs[:, 5]
...
def from_n_class_to_top_class(input):
...
def postprocess(
        ...
        prediction_confidence_threshold: float = None,
        iou_threshold: float = None,
        height: int = None,
        width: int = None,
        k: int = None,
        convert_coco80_to_coco91=False
):
    """
    Post-process an image detection model.

    Parameters
    ----------
    decoded_outputs
        List of outputs for a batch, where each output is
    ...
    convert_coco80_to_coco91
        If classes should be converted from coco80 to coco91.
        For efficientdet or any other models not pretrained on coco-detection this should be False
        (efficientdet was pretrained on coco-detection-90).
    ...
    # Deal with different input format when model is processed though leip-models
    is_leip = False
    if isinstance(decoded_outputs, list) and isinstance(decoded_outputs[0], ndarray):
        assert len(decoded_outputs) == 1, f"there is more than one model output, but the general postprocessor you are using expects a single model output"
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        decoded_outputs = torch.from_numpy(decoded_outputs[0]).to(device) # only the first element of the list is needed, it also requires numpy -> torch.tensor conversion
        is_leip = True

    # Do the Postprocessing: top K filtering -> NMS -> confidence based filtering
    batch_detections = post_process(decoded_outputs, iou_threshold, max_det_per_image, prediction_confidence_threshold, k)
